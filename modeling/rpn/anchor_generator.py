...
class AnchorGenerator(nn.Module):
    ...
    def grid_anchors(self, grid_sizes):
        ...
        for size, stride, base_anchors in zip(
            ...
            #shifts_x = torch.ones(grid_width * stride).nonzero().view(grid_width, stride).to(
            #    torch.float32).index_select(1, torch.zeros(1, dtype=torch.long)).to(device)
            #shifts_y = torch.ones(grid_height * stride).nonzero().view(grid_height, stride).to(
            #    torch.float32).index_select(1, torch.zeros(1, dtype=torch.long)).to(device)
