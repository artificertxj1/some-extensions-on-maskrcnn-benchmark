...
## this one is a little different from the pooler above
class PAN_Pooler(nn.Module):
    def __init__(self, output_size, scales, sampling_ratio):
        super(PAN_Pooler, self).__init__()
        poolers = []
        for scale in scales:
            poolers.append(
                ROIAlign(
        ...
        ##panet pooler doesn't use a lvl mapper
    ...
    def forward(self, x, boxes):
        ...
        :param boxes: list[BoxList]
        :return:
            extracted feature Tensors (Tensors)
        """
        num_levels = len(self.poolers)
        rois = self.convert_to_roi_format(boxes)
        if num_levels == 1:
